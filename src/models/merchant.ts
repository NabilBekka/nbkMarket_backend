...
export interface Merchant { id: string; email: string; password_hash: string | null; first_name: string; last_name: string; company_name: string; category_id: number | null; wilaya_code: number | null; profile_image: string | null; cover_image: string | null; address: string | null; description: string | null; sells_buys: boolean; offers_services: boolean; has_physical_shop: boolean; offers_delivery: boolean; reset_code: string | null; reset_expires: string | null; refresh_token: string | null; lang: string; created_at: string; updated_at: string; }
...
export async function createMerchant(data: { email: string; password_hash: string | null; first_name: string; last_name: string; company_name: string; category_id?: number | null; wilaya_code?: number | null; profile_image?: string | null; cover_image?: string | null; address?: string | null; description?: string | null; sells_buys?: boolean; offers_services?: boolean; has_physical_shop?: boolean; offers_delivery?: boolean; lang?: string }): Promise<Merchant> {
  const { rows } = await pool.query(
    `INSERT INTO merchants (email, password_hash, first_name, last_name, company_name, category_id, wilaya_code, profile_image, cover_image, address, description, sells_buys, offers_services, has_physical_shop, offers_delivery, lang)
