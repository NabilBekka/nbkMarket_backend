...
export interface PendingMerchant { id: string; email: string; password_hash: string; first_name: string; last_name: string; company_name: string; category_id: number | null; wilaya_code: number | null; sells_buys: boolean; offers_services: boolean; has_physical_shop: boolean; offers_delivery: boolean; delivery_wilayas: string; lang: string; verification_code: string; verification_expires: string; created_at: string; }
export async function create(data: { email: string; password_hash: string; first_name: string; last_name: string; company_name: string; category_id?: number | null; wilaya_code?: number | null; sells_buys?: boolean; offers_services?: boolean; has_physical_shop?: boolean; offers_delivery?: boolean; delivery_wilayas?: string; lang: string; verification_code: string; verification_expires: Date }): Promise<PendingMerchant> {
  const { rows } = await pool.query(
    `INSERT INTO pending_merchant_registrations (email, password_hash, first_name, last_name, company_name, category_id, wilaya_code, sells_buys, offers_services, has_physical_shop, offers_delivery, delivery_wilayas, lang, verification_code, verification_expires)
